...
$check_update = false;
if ($jew_oldinfo["jew_name"] != $jew_name) {
    $query_update .= "jew_name = '$jew_name', "; 
    $check_update = true;
}
if ($jew_oldinfo["jew_price"] != $jew_price) {
    $query_update .= "jew_price = $jew_price, ";
    $check_update = true;
}
if ($jew_oldinfo["jew_descr"] != $jew_descr) {
    $query_update .= "jew_descr = '$jew_descr', ";
    $check_update = true;
}
if ($jew_oldinfo["jew_cat"] != $category_re["category_id"]) {
    $category_id = $category_re['category_id'];
    $query_update .= "jew_cat = $category_id, ";
        $check_update = true;
}
if ($jew_img) {
    $query_update .= "jew_img ='".$jew_img['name']."', ";
    $check_update = true;
    move_uploaded_file($jew_img["tmp_name"], "../images/jew/$jew_img[name]");
}
